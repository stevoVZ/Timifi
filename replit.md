...
-   **Admin Panel:** Modules for Dashboard, Employee management (profiles, financials), Timesheet processing (AI OCR upload, review, approval, audit trails, monthly hours editor), Payroll (pay run management, payslips, ABA files), Invoicing (multi-employee linkage, Invoice Alignment Wizard for auto-matching), RCTIs (Recipient Created Tax Invoices management), Reconciliation (interactive cells, quick edits, contractor cost tracking, month completeness KPI), Bank Statements (synced from Xero with linkage status), Profitability (revenue vs. cost per employee-client placement, three-tier hours model, drill-down capabilities, client ledger with estimated revenue), Notifications, and Settings.
