...
-   **Schema:** Key tables include `employees`, `timesheets`, `invoices` (with `client_id` and `xero_contact_id` for Xero contact-based client linking), `pay_runs`, `pay_run_lines`, `documents`, `notifications`, `messages`, `settings`, `users`, `leave_requests`, `pay_items`, `tax_declarations`, `bank_accounts`, `super_memberships`, `clients` (with `is_rcti` flag), `placements`, `bank_transactions`, `payslip_lines`, `rate_history`, `timesheet_audit_log`, `invoice_employees`, and `rctis`.
