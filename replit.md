...
-   **Referral Bonus System:** Admin staff or employees who refer workers that get placed can earn ongoing referral bonuses. Managed via `referral_bonuses` table with referring/referred employee IDs, bonus rate per hour, start/end dates, and status (ACTIVE/ENDED). Bonus amounts are calculated as referred employee's best-available hours × bonus rate and appear as cost-only rows in the profitability report with a "REFERRAL_BONUS" row type and purple visual indicator. The employee detail page (Profile tab) has a Referral Bonuses card for creating, viewing, and ending referral arrangements. API: `GET/POST /api/referral-bonuses`, `GET /api/referral-bonuses/employee/:id`, `PATCH /api/referral-bonuses/:id`, `POST /api/referral-bonuses/:id/deactivate`. Ended bonuses with end dates still appear in historical profitability reports for the months they were active.
