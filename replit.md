...
-   **Admin Panel:** Includes comprehensive modules for Dashboard, Employees (with detailed profiles, financials, documents), Timesheets (upload with AI OCR, review, approval workflow, monthly hours editor for inline editing), Payroll (pay run management, payslip generation, ABA files), Invoices (with detail dialog showing employee linkage and editable assignment), Reconciliation (interactive cells linking to related records, quick edit dialog for timesheet hours), Bank Statements (synced from Xero), Profitability (revenue vs cost per employee-client placement with auto-link invoices), Client Ledger (client payments received vs employee costs paid with flexible date ranges and drill-down), Notifications, Settings (including Xero integration), Clients, and Placements.
